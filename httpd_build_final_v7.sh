...
# v7.0 (2026-04-11): mod_jk 메인 빌드 통합
#                - build_httpd() → build_httpd_with_mod_jk() 통합
#                - httpd 설치 후 즉시 mod_jk 빌드 (같은 함수 내)
#                - build_mod_jk() 별도 함수 제거 → 메인 빌드 흐름 단순화
# =============================================================================
...
# [v7.0에서 제거됨] build_httpd_with_mod_jk()에 통합됨
# build_mod_jk() {
#   log "Building mod_jk-${MOD_JK_VERSION}"
#   ...
# }
...
_fix_apachectl() {
  ctl="${APP_HOME}/bin/apachectl"
  [ -f "${ctl}" ] || return 0
  grep -qF 'HTTPD_ROOT=' "${ctl}" && \
    { echo "INFO: apachectl 이미 수정됨 (skip)"; return 0; }
