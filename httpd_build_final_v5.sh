...
# v3.1 (현재)  : mod_jk 1.2.50 추가
#                - Tomcat AJP 연동 대비 mod_jk.so 번들 (LoadModule 주석 처리 기본)
#                - conf/workers.properties.minimal 샘플 포함
# v4.0         : 런타임 스크립트 이식성 개선
#                - env/start/stop/restart/status 스크립트 sh 호환화
#                - relocate_apache_paths.sh 에서 bash 전용 구문 제거 (sh 호환)
#                - DocumentRoot/htdocs 경로 리로케이션 보강
#                - sed -i 의존 제거(Perl 치환)로 GNU/BSD sed 차이 회피
# =============================================================================
...
OUT_TAR="${BASE}/httpd-${HTTPD_VERSION}-glibc2.25-compiled-v5.tar.gz"
